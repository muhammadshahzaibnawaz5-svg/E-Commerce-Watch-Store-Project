...
window.handleSignIn = async function() {
  ...
  if (!auth) {
    ...
    if (loadingBox) { loadingBox.classList.add('hidden') }
    if (successBox) {
      successBox.textContent = 'Signed in! Redirecting...'
      successBox.classList.remove('hidden')
    }
    setTimeout(() => {
      const returnTo = sessionStorage.getItem('signin_return') || 'index.html'
  ...
  try {
    ...
    if (error.code === 'auth/email-already-in-use') {
      try {
        ...
        if (successBox) {
          successBox.textContent = 'Welcome back! Redirecting...'
          successBox.classList.remove('hidden')
        }
        ...
        if (errorBox) {
          errorBox.textContent = 'Email already registered. Please sign in with your password.'
      ...
      if (errorBox) { errorBox.textContent = 'Password is too weak. Use a stronger password.'; errorBox.classList.remove('hidden') }
    } else {
...
window.handleFirebaseSignIn = async function() {
  const emailInput = document.getElementById('signin-email')
  const passwordInput = document.getElementById('signin-password')
  const errorBox = document.getElementById('signin-error')
  const loadingBox = document.getElementById('signin-loading')
  ...
  if (!email || !password) {
    if (errorBox) { errorBox.textContent = 'Please enter email and password.'; errorBox.classList.remove('hidden') }
    if (loadingBox) { loadingBox.classList.add('hidden') }
    return
  }
  ...
  try {
    ...
    if (error.code === 'auth/invalid-email' || error.code === 'auth/wrong-password' || error.code === 'auth/user-not-found') {
      if (errorBox) { errorBox.textContent = 'Invalid email or password.'; errorBox.classList.remove('hidden') }
    } else {
      if (errorBox) { errorBox.textContent = 'Error: ' + error.message; errorBox.classList.remove('hidden') }
    }
  }
...
function loadAdminProducts() {
  try {
    ...
    adminAdded.forEach(function(product) {
      ...
      card.innerHTML =
        '<img class="h-40 mx-auto mb-6 object-contain" src="' + product.image + '" alt="' + product.name + '" onerror="this.src=\'/src/Components/images/w1.png\'">' +
...
const initialProducts = [
  { id: 1, name: "Men's Watch", category: "Men", price: 300, image: "/src/Components/images/w1.png" },
  { id: 2, name: "Men's Watch", category: "Men", price: 300, image: "/src/Components/images/w2.png" },
  { id: 3, name: "Men's Watch", category: "Men", price: 300, image: "/src/Components/images/w3.png" },
  { id: 4, name: "Men's Watch", category: "Men", price: 300, image: "/src/Components/images/w4.png" },
  { id: 5, name: "Men's Watch", category: "Men", price: 300, image: "/src/Components/images/w5.png" },
  { id: 6, name: "Men's Watch", category: "Men", price: 300, image: "/src/Components/images/w6.png" },
  { id: 7, name: "Female Watch", category: "Female", price: 200, image: "/src/Components/images/w10.png" },
  { id: 8, name: "Female Watch", category: "Female", price: 200, image: "/src/Components/images/w11.png.jpg" },
  { id: 9, name: "Female Watch", category: "Female", price: 200, image: "/src/Components/images/w12.png.jpg" },
  { id: 10, name: "Digital Watch", category: "Digital", price: 400, image: "/src/Components/images/w19.png" },
  { id: 11, name: "Smart Watch", category: "Smart", price: 500, image: "/src/Components/images/w22.png" },
  { id: 12, name: "Digital Watch", category: "Digital", price: 400, image: "/src/Components/images/d1.png" }
]
...
function showSaveToast() {
  ...
  toast.innerHTML = '<i class="fa-solid fa-circle-check"></i> Saved — Products page updated!'
  ...
  filteredProducts.forEach(product => {
    ...
    row.innerHTML = `
      ...
      <td class="py-5 text-center">
        <button onclick="editProduct(${product.id}); event.stopImmediatePropagation()" 
                class="text-violet-600 hover:text-violet-700 px-3 py-1 rounded-2xl hover:bg-violet-50">
          <i class="fa-solid fa-pen"></i>
        </button>
        <button onclick="deleteProduct(${product.id}); event.stopImmediatePropagation()" 
                class="text-red-500 hover:text-red-600 px-3 py-1 rounded-2xl hover:bg-red-50 ml-2">
...
function showAddProductModal() {
  ...
  document.getElementById('image-preview').src = '/src/Components/images/w1.png'
...
function handleProductForm(e) {
  ...
  if (currentEditingId !== null) {
    ...
    const newProduct = {
      ...
      category: category,
      price: price,
      image: tempImageDataUrl || '/src/Components/images/w1.png'
    }
...
window.onload = function() {
  // Initialize user auth (works on all pages)
  initUserAuth()
  ...
  if (loginOverlay) {
    // This is admin page - load products and setup
    loadProducts()
    showPanel('dashboard')
    
    // If already authenticated this session, skip the login screen
